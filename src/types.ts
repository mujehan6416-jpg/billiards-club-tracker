...
export interface Game {
  ...
  /**
   * 이 결과를 입력한 주체의 역할. 'admin'은 "관리자 모드에서 입력됨"이라는 뜻이며 특정
   * 관리자 개인을 가리키지 않는다(관리자 판정은 기기 PIN 기반이라 신원을 보증하지 못한다).
   * 이 필드가 없던 기존 경기는 입력자를 표시하지 않는다(마이그레이션하지 않는다).
   */
  submittedByRole?: 'admin' | 'member'
  /**
   * 회원이 입력한 경우 그 회원의 ID(Member.id). 이름은 저장하지 않고 회원 명부에서 찾아
   * 표시한다 — 개인정보를 경기 문서에 중복 저장하지 않기 위해서다.
   *
   * 값의 기준은 "앱에서 고른 이름"이 아니라 memberLinks(기기-회원 연결) 기준이다. 보안
   * 규칙도 linkedMemberId()와 같은지 검사하므로 남의 ID로는 저장되지 않는다.
   */
