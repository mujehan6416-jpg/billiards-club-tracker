...
// 정기모임 정산·모임(경기결과)·대회 토너먼트 개발용 임시 진입점. import.meta.env.DEV는 운영
// 빌드에서 항상 false로 치환되므로 운영 배포에는 절대 나타나지 않는다. 제거할 때는 각 블록과
// src/dev/ 폴더만 지우면 된다.
...
const isDevTournamentPreview = viteEnv.DEV && params.get('devTournament') === '1'
...
const DevTournamentPreview = isDevTournamentPreview ? lazy(() => import('./dev/DevTournamentPreview')) : null
