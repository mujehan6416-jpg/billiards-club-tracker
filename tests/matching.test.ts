...
describe('canRematchRound — 1·2라운드 재매칭 정책 통일(공용 함수, 라운드별 차별 없음)', () => {
  it('1라운드 결과가 없으면 재매칭을 허용한다', () => {
    const s = session({ games: [], lineup: [{ round: 1, aId: 'A', bId: 'B', handicapA: 20, handicapB: 20 }] })
    expect(canRematchRound(s, 1)).toBe(true)
  })

  it('1라운드 결과가 하나라도 저장되면 재매칭을 차단한다(2라운드와 동일한 정책)', () => {
    const s = session({ games: [game('A', 'B', { round: 1, scoreA: 15, scoreB: 10 })] })
    expect(canRematchRound(s, 1)).toBe(false)
  })

  it('2라운드 결과는 1라운드 재매칭 가능 여부에 영향을 주지 않는다', () => {
    const s = session({ games: [game('A', 'B', { round: 2, scoreA: 20, scoreB: 5 })] })
    expect(canRematchRound(s, 1)).toBe(true)
  })

  it('pending(관리자 확인 필요) 결과가 있어도 이미 점수가 기록된 것이므로 재매칭을 차단한다(1·2라운드 공통)', () => {
    const s1 = session({ games: [game('A', 'B', { round: 1, scoreA: 12, scoreB: 8, pending: true })] })
    const s2 = session({ games: [game('A', 'B', { round: 2, scoreA: 12, scoreB: 8, pending: true })] })
    expect(canRematchRound(s1, 1)).toBe(false)
    expect(canRematchRound(s2, 2)).toBe(false)
  })

  it('확인 완료(pending:false) 결과가 있어도 재매칭을 차단한다(1·2라운드 공통)', () => {
    const s1 = session({ games: [game('A', 'B', { round: 1, scoreA: 20, scoreB: 18, pending: false })] })
    const s2 = session({ games: [game('A', 'B', { round: 2, scoreA: 20, scoreB: 18, pending: false })] })
    expect(canRematchRound(s1, 1)).toBe(false)
    expect(canRematchRound(s2, 2)).toBe(false)
  })

  it('수정 요청(revisionRequested) 상태의 결과가 있어도 이미 점수가 기록돼 있으므로 재매칭을 차단한다(1·2라운드 공통)', () => {
    const s1 = session({ games: [game('A', 'B', { round: 1, scoreA: 10, scoreB: 9, pending: true, revisionRequested: true })] })
    const s2 = session({ games: [game('A', 'B', { round: 2, scoreA: 10, scoreB: 9, pending: true, revisionRequested: true })] })
    expect(canRematchRound(s1, 1)).toBe(false)
    expect(canRematchRound(s2, 2)).toBe(false)
  })

  it('참가자 변경(attendeeIds 추가) 후에도 결과가 없으면 재매칭이 계속 허용된다', () => {
    const s = session({ games: [], attendeeIds: ['A', 'B'] })
    const changed = { ...s, attendeeIds: ['A', 'B', 'C'] }
    expect(canRematchRound(changed, 1)).toBe(true)
  })

  it('참가자 변경 후에도 결과가 있으면 여전히 차단된다', () => {
    const s = session({ games: [game('A', 'B', { round: 1, scoreA: 20, scoreB: 5 })], attendeeIds: ['A', 'B'] })
    const changed = { ...s, attendeeIds: ['A', 'B', 'C'] }
    expect(canRematchRound(changed, 1)).toBe(false)
  })
})
