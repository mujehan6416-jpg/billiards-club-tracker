...
describe('금액 입력칸 너비 — index.css의 input[type=number]{width:64px} 전역 규칙을 인라인으로 덮어쓴다', () => {
  // jsdom은 실제 CSS 레이아웃을 계산하지 않으므로(실제 픽셀 렌더링 검증은 브라우저에서 별도 확인),
  // 여기서는 "너비를 강제로 넓히는 인라인 스타일이 실제로 적용돼 있는지"만 회귀 테스트로 고정한다.
  it('지출 "금액" 입력칸이 min-width 확보 스타일을 갖는다(64px보다 넓게)', () => {
    render(<SettlementExpenseForm settlementId="settle-expense-1" onRequestDinnerForm={() => {}} />)
    // 금액/모임부담액/개인찬조액 모두 placeholder="0"을 공유한다 — 첫 번째가 "금액" 필드.
    const [amountInput] = screen.getAllByPlaceholderText('0') as HTMLInputElement[]
    expect(amountInput.style.minWidth).toBe('130px')
    expect(amountInput.style.width).toBe('100%')
  })

  it('지출 "모임 부담액"·"개인 찬조액" 입력칸도 min-width가 확보된다(64px보다 넓게)', () => {
    render(<SettlementExpenseForm settlementId="settle-expense-1" onRequestDinnerForm={() => {}} />)
    const [, clubShareInput, personalDonationInput] = screen.getAllByPlaceholderText('0') as HTMLInputElement[]
    expect(clubShareInput.style.minWidth).toBe('100px')
    expect(personalDonationInput.style.minWidth).toBe('100px')
  })

  it('회식비 "전체 회식비" 입력칸이 min-width 확보 스타일을 갖는다', () => {
    render(<DinnerContributionForm settlementId="settle-expense-1" />)
    const input = screen.getByPlaceholderText('0') as HTMLInputElement
    expect(input.style.minWidth).toBe('130px')
    expect(input.style.width).toBe('100%')
  })

  it('회식비 "찬조자 금액" 입력칸도 기존 100px보다 넓게 확보된다', () => {
    render(<DinnerContributionForm settlementId="settle-expense-1" />)
